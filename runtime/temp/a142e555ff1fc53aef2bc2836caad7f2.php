<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:90:"D:\phpStudy\PHPTutorial\WWW\fsdfsdf\public/../application/admin\view\admin_user\index.html";i:1547805898;}*/ ?>
        ...
        <div class="lfnav_text clearfix">
            ...
            <ul id='bu_liist'>
                    <!-- showment 方法  /admin/admin_user/showment.html  code =>'0' 成功 $list  -->
        ...
        <div class="rftext_con clearfix" id="list">
            ...
            <!-- memberexport 方法  /admin/admin_user/memberexport.html  code =>'0'   -->
            <div class="daochu clearfix">
                <input type="hidden" value="4" name="ment_id">
                <span onclick="expord('4');" target="_blank">导出为Excel</span>
            </div>
            ...
            <div class="bot clearfix">
                <table border="" cellspacing="" cellpadding="">
                    ...
                    <!-- 展示成员 -->
                    <!-- showdepartment 方法  /admin/admin_user/showdepartment.html  code =>'0'  $departlist   -->
                    <tr>
                        ...
                        <td><span>激活</span></td>
                        <td>
                            <button onclick="reset_password('传uid')">重置密码</button>
                            <!-- 传id -->
                            <a class="edit_a" data-href="<?php echo \think\Url::build('admin_user/editmembera'); ?>&uid=3" data-nav="<?php echo \think\Url::build('admin_user/nav'); ?>">修改</a>
                            <!--  -->
                            <button onclick="frozen_accoun('传uid')">冻结</button>

                            <button class="del" onclick="delete_members('传uid')" >删除</button>
                        </td>
                    </tr>
                    <tr>
                        <td><img src="img/grzx_02jpg.jpg"/></td>
                ...
                <!-- <form> -->
                    <div class="sc_div">
                        <ol class="bm_ol" id="if_gdt">
                            <h2>选择部门:</h2>
                            <!--  -->
                            <!-- 删除部门 deletedepartment("传deletedepartment")  -->

                            <!-- 展示部门 -->
                            <!-- showment 方法  /admin/admin_user/showment.html ； code =>'0' 成功； $list => 所有部门；$parentlist =>父类部门 -->

                            <li ondrop="drop(event,this)" ondragover="allowDrop(event)" draggable="true" ondragstart="drag(event, this)" onclick="deletedepartment('');">
                    ...
                    <input class="qd_in" type="submit"  draggable="true" value="确定" onclick="insertment();" />
...
function insertment(){
            $.ajax({
                ...
                type:"post",
                data: $('#if_gdt input[type=\'text\']','#if_gdt input[type=\'hidden\']'),
                // success:function(){
                //        location.reload(); 
                // }
            });
...
function deletedepartment(department_id){
    // alert(11);
            $.ajax({
                url:'<?php echo \think\Url::build("delment"); ?>',
                type:"post",
...
//删除成员
function delete_members(uid){
            $.ajax({
                url:'<?php echo \think\Url::build("delmember"); ?>',
                type:"post",
                data: {'uid':uid},
                // success:function(){
                //        location.reload(); 
...
function reset_password(uid){
            $.ajax({
                url:'<?php echo \think\Url::build("resetpassword"); ?>',
                type:"post",
                data: {'uid':uid},
                // success:function(){
                //        location.reload(); 
...
//冻结账户
function frozen_accoun(uid){
            $.ajax({
                url:'<?php echo \think\Url::build("frozenmember"); ?>',
                type:"post",
                data: {'uid':uid},
                // success:function(){
                //        location.reload(); 
...
//导出
function expord(department_id){
    // alert(department_id);
            $.ajax({
                url:'<?php echo \think\Url::build("memberexport"); ?>',
                type:"post",
                data: {'department_id':department_id},
                // success:function(){
                //        location.reload(); 
